...
# Лабораторна 2 рівень 3: Про максимальну кількість хом'яків
...
# Лабораторна 3 рівень 3: Пошук наступника в бінарному дереві
...
# Лабораторна 4 рівень 3: Черга з пріоритетом на основі червоно-чорного дерева
...
# Лабораторна 5 рівень 3: Про шагового коня
...
markdown
Копіювати код
# Лабораторна 6 рівень 3: Про ігровий сервер

### Опис:
Ігровий сервер 88889 Код задачі: GAMSRV. Важливим фактором для багатокористувацької онлайн-гри є низька мережева затримка від користувача до сервера. При цьому, пристрої в Інтернеті спілкуються один з одним, використовуючи мережеві маршрути, які проходять через низку проміжних вузлів-маршрутизаторів. Кожна ланка цього маршруту має власну ненульову затримку.
...
Server може бути лише один на всю мережу. Усі комунікації двосторонні: якщо вузол A може спілкуватися з вузлом B, вузол B може спілкуватися з вузлом A з такою ж затримкою. Якщо існує кілька шляхів від клієнта до сервера, клієнт гарантовано піде шляхом з найменшою сумарною затримкою (навіть якщо цей шлях пролягає через іншого клієнта). Усі затримки — сталi додатнi числа.
...
### Вихідні дані:
Вихідний файл `gamsrv.out` повинен містити одне число — мінімальне значення найбільшої затримки до 

Лабораторна 7. Скінченні автомати
markdown
Копіювати код
# Лабораторна 7 рівень 2: Скінченні автомати
...
```markdown
# Лабораторна 8 рівень 2: З'єднати всі острови оптоволокном у Венеції
...
### Вихідні дані:
Мінімальна довжина підводних кабелів, які потрібно прокласти. При виборі алгоритму слід вважати, що кількість островів у м

Лабораторна 9. Індіана Джонс В пошуках Святого Грааля
markdown
Копіювати код
# Лабораторна 9 рівень 2: Індіана Джонс В пошуках Святого Грааля
